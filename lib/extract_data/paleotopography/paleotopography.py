...
# sys.path.insert(0, os.path.join(DIRNAME, "..", "paleogeography"))
...
def get_distance_to_mountain_edge(
    point_array,
    reconstruction_basedir,
    rotation_model,
    time,
    area_threshold,
    present_day_filename="present_day_paleogeography.gmt",
):
    ...
    if time==0:
        ...
        pg_features = load_paleogeography(
            present_day_filename,
            env_list,
            env_field="Layer",
        )
    else:
        ...
        pg_features = load_paleogeography(reconstruction_basedir, env_list, time=time)
...
def get_merged_cob_terrane_polygons(COBterrane_file, rotation_model, reconstruction_time,
    ...
    polygon_features = pygplates.FeaturesFunctionArgument(COBterrane_file).get_features()
    ...
    cf = []
    tmp = []
    pygplates.reconstruct(cobter, rotation_model, tmp, float(reconstruction_time))
    for i in tmp:
        feature = i.get_feature()
        geom = i.get_reconstructed_geometry()
        new = feature.clone()
        new.set_geometry(geom)
        cf.append(new)
    del tmp
...
def paleotopography_job(reconstruction_time, paleogeography_timeslice_list, 
    ...
    # sieve_polygons = split_polygons(sieve_polygons)
    # sieve_polygons.write("sieve_polygons_split.gpml")
    # pygplates.reconstruct(
    #     sieve_polygons,
    #     rotation_model,
    #     "sieve_polygons_split.shp",
    #     float(0.0),  # sieve polygons are in reconstructed positions
    # )
    ...
    if np.equal(reconstruction_time, time_stage_min):
        ...
        dist_tr = get_distance_to_mountain_edge(mountains_tr_point_array, reconstruction_basedir,
                                                ...
                                                present_day_filename=present_day_filename)
    ...
    with \
        tempfile.NamedTemporaryFile() as land_marine_xyz_file,\
        tempfile.NamedTemporaryFile() as mountain_xyz_file,\
        tempfile.NamedTemporaryFile() as land_marine_nc_file,\
        tempfile.NamedTemporaryFile() as mountain_nc_file:
